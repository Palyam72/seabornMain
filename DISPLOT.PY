...
class DisPlot:
    ...
    def display(self):
        ...
        with tab1:
            ...
            with col1:
                self.x = st.selectbox("Select the column for x-axis", [None] + self.columns, key="x_axis_distplot")
                self.y = st.selectbox("Select the column for y-axis", [None] + self.columns, key="y_axis_distplot")
                self.hue = st.selectbox("Select the column for hue", [None] + self.columns, key="hue_distplot")
                self.row = st.selectbox("Facet by rows", [None] + self.columns, key="row_distplot")
                self.col = st.selectbox("Facet by columns", [None] + self.columns, key="col_distplot")
                if self.col:
                    self.col_wrap = st.number_input("Wrap columns at specified width", min_value=1, max_value=5, value=3, key="col_wrap_distplot")
                else:
                    self.col_wrap=None
                self.weights = st.selectbox("Select weights column (optional)", [None] + self.columns, key="weights_distplot")
                self.kind = st.selectbox("Select plot type", ["hist", "kde", "ecdf"], key="kind_distplot")
                self.rug = st.checkbox("Enable Rugplot", key="rug_distplot")
                self.legend = st.checkbox("Show Legend", key="legend_distplot")
                self.palette = st.selectbox("Select a color palette", ["deep", "muted", "pastel", "dark", "colorblind", "viridis", "coolwarm"], key="palette_distplot")
                ...
                self.log_scale=st.checkbox("Select the log scale")
        ...
        if st.button("Generate Distplot", use_container_width=True, type='primary', key="plot_button_distplot"):
            try:
                fig = sns.displot(
                    data=self.data, x=self.x, y=self.y, hue=self.hue, weights=self.weights, kind=self.kind,log_scale=self.log_scale if self.log_scale else None,
                    rug=self.rug, legend=self.legend, palette=self.palette, hue_order=self.hue_order,
